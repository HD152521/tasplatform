...
function deleteWithAudit(
  db: ReturnType<typeof openDb>,
  args: { actor: string; teamId: string; kind: string },
): void {
  deleteIntegration(db, args.teamId, args.kind);
  recordAudit(db, {
...
test("연동 등록 성공 시 audit_log 에 action=integration_upsert, team_id, result=ok 가 남는다", () => {
...
test("actor 를 안 보내면 빈 문자열로 기록된다", () => {
  const { db, cleanup } = tempDb();
  try {
    withKey(KEY, () => {
      postUpsert(db, {
    ...
    assert.equal(listAudit(db, 10)[0]?.actor, "");
  } finally { cleanup(); }
});

test("SR_SECRET_KEY 가 없어 저장이 실패하면 result=failed:no_key 로 남는다", () => {
  const { db, cleanup } = tempDb();
  try {
    withKey(undefined, () => {
      assert.throws(() => postUpsert(db, {
        actor: "user@corp.com", teamId: DEFAULT_TEAM_ID, kind: "jira",
        baseUrl: "https://a.atlassian.net", project: "ABC", secret: "tok",
      }));
    });
...
test("존재하지 않는 팀이라 저장이 실패하면 result=failed:error 로 남는다", () => {
...
test("연동 삭제 시 audit_log 에 action=integration_delete, team_id, result=ok 가 남는다", () => {
  const { db, cleanup } = tempDb();
  try {
    withKey(KEY, () => {
      postUpsert(db, {
    ...
    deleteWithAudit(db, { actor: "user2@corp.com", teamId: DEFAULT_TEAM_ID, kind: "jira" });
...
test("audit detail 에는 kind 만 남고 base_url·secret 값은 절대 들어가지 않는다", () => {
  const { db, cleanup } = tempDb();
  try {
    ...
    deleteWithAudit(db, { actor: "user@corp.com", teamId: DEFAULT_TEAM_ID, kind: "jira" });
