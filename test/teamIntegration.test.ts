...
test("upsert 후 getIntegration 으로 복호화된 값을 읽을 수 있다 (왕복)", () => {
  const { db, cleanup } = tempDb();
  try {
    withKey(KEY, () => {
      upsertIntegration(db, {
        teamId: DEFAULT_TEAM_ID, kind: "jira",
        baseUrl: "https://example.atlassian.net/", project: "ABC", secret: "tok-123",
      });
      const got = getIntegration(db, DEFAULT_TEAM_ID, "jira");
      assert.ok(got);
...
test("SR_SECRET_KEY 가 없으면 토큰이 있는 저장을 거부한다 (평문 저장 금지, 부분 저장도 없음)", () => {
  const { db, cleanup } = tempDb();
  try {
    withKey(undefined, () => {
      assert.throws(
    ...
    const rows = db.prepare("SELECT * FROM team_integrations").all();
    assert.equal(rows.length, 0);
  } finally { cleanup(); }
});

test("secret 없이(빈 값) 저장하는 것은 허용된다 — 키가 없어도 등록만은 가능하다", () => {
  const { db, cleanup } = tempDb();
  try {
    withKey(undefined, () => {
      assert.doesNotThrow(() => upsertIntegration(db, {
        teamId: DEFAULT_TEAM_ID, kind: "jira",
        baseUrl: "https://example.atlassian.net", project: "ABC", secret: "",
      }));
    });
    assert.equal(listIntegrations(db, DEFAULT_TEAM_ID)[0]?.hasSecret, false);
  } finally { cleanup(); }
});

test("secret 을 비워 두고 수정하면 기존 토큰이 유지된다", () => {
  const { db, cleanup } = tempDb();
  try {
    withKey(KEY, () => {
      upsertIntegration(db, {
...
test("listIntegrations 는 평문·암호문을 노출하지 않고 존재 여부만 돌려준다", () => {
  const { db, cleanup } = tempDb();
  try {
    withKey(KEY, () => {
      upsertIntegration(db, {
    ...
    const list = listIntegrations(db, DEFAULT_TEAM_ID);
    ...
    const rawRow = db
      .prepare("SELECT secret_enc FROM team_integrations WHERE team_id = ? AND kind = 'jira'")
      .get(DEFAULT_TEAM_ID) as { secret_enc: string };
    assert.ok(!serialized.includes(rawRow.secret_enc));
  } finally { cleanup(); }
});

test("토큰 없이도 등록할 수 있고(hasSecret=false), 이후 토큰을 추가할 수 있다", () => {
  const { db, cleanup } = tempDb();
  try {
    upsertIntegration(db, {
      teamId: DEFAULT_TEAM_ID, kind: "jira",
    ...
    assert.equal(listIntegrations(db, DEFAULT_TEAM_ID)[0]?.hasSecret, true);
  } finally { cleanup(); }
});

test("getIntegration 은 없는 연동에 대해 null 을 돌려준다", () => {
  const { db, cleanup } = tempDb();
  try {
    assert.equal(getIntegration(db, DEFAULT_TEAM_ID, "jira"), null);
  } finally { cleanup(); }
});

test("deleteIntegration 은 해당 팀·kind 행만 지운다", () => {
  const { db, cleanup } = tempDb();
  try {
    ...
    deleteIntegration(db, DEFAULT_TEAM_ID, "jira");
    assert.equal(listIntegrations(db, DEFAULT_TEAM_ID).length, 0);
    assert.equal(listIntegrations(db, "acme").length, 1);
  } finally { cleanup(); }
});

test("존재하지 않는 연동을 지워도 조용히 반환한다", () => {
  const { db, cleanup } = tempDb();
  try {
    assert.doesNotThrow(() => deleteIntegration(db, DEFAULT_TEAM_ID, "jira"));
  } finally { cleanup(); }
});

test("잘못된 형식의 teamId(경로 탈출 시도)는 거부한다", () => {
  const { db, cleanup } = tempDb();
  try {
    assert.throws(() => upsertIntegration(db, {
      teamId: "../x", kind: "jira", baseUrl: "https://a.atlassian.net", project: "A", secret: "",
    }));
    assert.throws(() => listIntegrations(db, "../x"));
    assert.throws(() => getIntegration(db, "../x", "jira"));
  } finally { cleanup(); }
});

test("존재하지 않는 팀으로는 등록을 거부한다", () => {
  const { db, cleanup } = tempDb();
  try {
    assert.throws(() => upsertIntegration(db, {
...
test("잘못된 base_url(http, 형식오류)은 거부한다", () => {
  const { db, cleanup } = tempDb();
  try {
    assert.throws(() => upsertIntegration(db, {
...
test("project 가 비어 있으면(공백만) 거부한다", () => {
  const { db, cleanup } = tempDb();
  try {
    assert.throws(() => upsertIntegration(db, {
...
test("서로 다른 팀은 같은 kind 를 독립적으로 저장한다", () => {
  const { db, cleanup } = tempDb();
  try {
    upsertTeam(db, { team_id: "acme", team_name: "Acme", broadcom_username: "" });
    withKey(KEY, () => {
      ...
      assert.equal(getIntegration(db, DEFAULT_TEAM_ID, "jira")?.secret, "t1");
      assert.equal(getIntegration(db, "acme", "jira")?.secret, "t2");
    });
  } finally { cleanup(); }
});
