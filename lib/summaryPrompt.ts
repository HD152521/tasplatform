...
# 원문 충실도 (가장 중요)
- 원문의 경고·에러·로그 메시지, UI 메뉴·버튼·필드·화면 이름, 제품 기능·옵션 이름은
  **원문 표기 그대로** 씁니다. 뜻이 통하게 한국어로 의역하거나 괄호 병기로 바꾸지 않습니다.
- 화면에 뜬 경고/에러 문구는 **큰따옴표로 원문 그대로 인용**합니다. 뜻만 풀어 쓰면 안 됩니다.
  나쁜 예: 인증서 목록에서 만료일이 75일 이상 남은 것으로 표시되나 경고가 발생합니다.
  좋은 예: Ops Manager UI 로그인 시 "Non-configurable leaf certificate가 75일 이내에 만료된다"는 경고가 표시됩니다.
- 증상은 **언제·어디서**(예: 로그인 시, 특정 메뉴 진입 시, 특정 작업 수행 중) 발생하는지가
  원문에 있으면 반드시 함께 적습니다. 발생 맥락을 지우고 결과만 요약하지 않습니다.
- 고객이 확인·요청한 구체 항목(예: "정상 동작 여부 확인 요청", "영향 범위 문의")은
  표현을 바꾸거나 통째로 빼지 않습니다. 요청의 핵심 낱말을 원문에 가깝게 옮깁니다.
- 더 상세히 풀어 쓰되, **원문에 없는 내용을 채워 넣지 않습니다(환각 금지)**. 상세함은
  문장을 늘리는 데서가 아니라 원문에 있는 사실·용어·인용을 빠짐없이 담는 데서 나옵니다.
...
- 원문의 경고·에러 문구, UI 메뉴·화면·기능 이름은 원문 표기 그대로 옮깁니다. 임의로
  한국어 의역하거나 괄호 병기(예: 설정 불가능한(non-configurable))로 바꾸지 않습니다.
  화면 경고 문구는 큰따옴표로 원문 그대로 인용합니다.
- 고객이 확인·요청한 구체 항목은 표현을 바꾸거나 빼지 않고 원문에 가깝게 옮깁니다.`;
...
export const CONFLUENCE_SECTIONS: readonly ConfluenceSection[] = [
  ...
  },
  {
    ...
    focus: [
      ...
      "증상이 언제·어디서(로그인 시, 특정 메뉴·화면에서) 나타나는지가 원문에 있으면 함께 적고, 화면에 뜬 경고·에러 문구는 큰따옴표로 원문 그대로 인용합니다. 뜻만 풀어 요약하지 않습니다.",
      "이어서 `상세 내용:` 아래에 무엇이 제약이었는지, 무엇을 확인·요청했는지를 항목으로 나눠 씁니다. 고객 요청의 구체 항목(예: \"정상 동작 여부 확인 요청\")은 표현을 바꾸거나 빼지 않습니다.",
    ].join("\n"),
  },
  {
    id: "diagnosis",
