...
test("teamId 로 그 팀 소유 케이스만 돌려준다", async () => {
  const { db, file, cleanup } = await tempDb();
  try {
    await upsertCase(db, sampleCase(1, "Open", "acme"));
    await upsertCase(db, sampleCase(2, "Open", "beta"));
    const result = await listCasesHandler("acme", {}, file);
...
test("scope=open/closed 로 걸러낸다", async () => {
  const { db, file, cleanup } = await tempDb();
  try {
    await upsertCase(db, sampleCase(1, "Open", "acme"));
    await upsertCase(db, sampleCase(2, "Closed", "acme"));
    const open = await listCasesHandler("acme", { scope: "open" }, file);
    const closed = await listCasesHandler("acme", { scope: "closed" }, file);
    assert.deepEqual(open.cases.map((c) => c.request_id), [1]);
    assert.deepEqual(closed.cases.map((c) => c.request_id), [2]);
  } finally { await cleanup(); }
});

test("scope 을 생략하면 전체를 돌려준다", async () => {
  const { db, file, cleanup } = await tempDb();
  try {
    await upsertCase(db, sampleCase(1, "Open", "acme"));
    await upsertCase(db, sampleCase(2, "Closed", "acme"));
...
test("limit·offset 으로 페이지를 나눈다", async () => {
  const { db, file, cleanup } = await tempDb();
  try {
    for (let i = 1; i <= 5; i += 1) await upsertCase(db, sampleCase(i, "Open", "acme", i));
    const page1 = await listCasesHandler("acme", { limit: 2, offset: 0 }, file);
    const page2 = await listCasesHandler("acme", { limit: 2, offset: 2 }, file);
...
test("빈 팀은 빈 목록을 돌려준다 (다른 팀 케이스가 있어도)", async () => {
  const { db, file, cleanup } = await tempDb();
  try {
    await upsertCase(db, sampleCase(1, "Open", "acme"));
    const result = await listCasesHandler("beta", {}, file);
    assert.equal(result.total, 0);
    assert.deepEqual(result.cases, []);
  } finally { await cleanup(); }
});

test("한도를 넘는 limit 은 상한(200)으로 잘린다", async () => {
  const { file, cleanup } = await tempDb();
  try {
    const result = await listCasesHandler("acme", { limit: 10_000 }, file);
    assert.equal(result.ok, true);
...
test("그 팀 소유 케이스는 상세·스레드·첨부를 돌려준다", async () => {
  const { db, file, cleanup } = await tempDb();
  try {
    await upsertCase(db, sampleCase(37074096, "Open", "acme"));
    const result = await getCaseHandler("acme", { requestId: 37074096 }, file);
    assert.equal(result.ok, true);
    if (!result.ok) return;
...
test("다른 팀 케이스는 못 찾음으로 취급한다 (팀 경계)", async () => {
  const { db, file, cleanup } = await tempDb();
  try {
    await upsertCase(db, sampleCase(1, "Open", "acme"));
    const result = await getCaseHandler("beta", { requestId: 1 }, file);
    assert.equal(result.ok, false);
  } finally { await cleanup(); }
});

test("존재하지 않는 requestId 는 못 찾음을 돌려준다", async () => {
  const { file, cleanup } = await tempDb();
  try {
    const result = await getCaseHandler("acme", { requestId: 999 }, file);
    assert.equal(result.ok, false);
  } finally { await cleanup(); }
});

test("requestId 가 숫자가 아니면 실패를 돌려준다", async () => {
  const { file, cleanup } = await tempDb();
  try {
    const result = await getCaseHandler("acme", { requestId: Number("not-a-number") }, file);
    assert.equal(result.ok, false);
  } finally { await cleanup(); }
});
...
test("기본 팀도 정상적으로 조회된다 (하위호환)", async () => {
  const { db, file, cleanup } = await tempDb();
  try {
    await upsertCase(db, sampleCase(1, "Open", DEFAULT_TEAM_ID));
    const result = await listCasesHandler(DEFAULT_TEAM_ID, {}, file);
