...
const NINE = {
  bank: { dev: 1, prod: 2, dr: 3 },
  central: { dev: 4, prod: 5, dr: 6 },
  shared: { dev: 7, prod: 8, dr: 9 },
};
...
function deps(over: Partial<ReportDeps> = {}): ReportDeps {
  return {
    ...
    resolvePrevious: async () => ({ values: {} as never, fromMonth: "2026-08" }),
...
test("month 는 YYYY-MM 만 받는다", () => {
  assert.ok(isMonthArg("2026-09"));
  assert.ok(!isMonthArg("2026-13"));
  assert.ok(!isMonthArg("2026-9"));
  assert.ok(!isMonthArg(202609));
});
...
test("빠진 칸은 이름으로 알려 준다", () => {
  const got = readCounts({ bank: { dev: 1 }, central: NINE.central, shared: NINE.shared });
  assert.ok("missing" in got);
  assert.deepEqual(got.missing, ["bank.prod", "bank.dr"]);
});

test("음수나 숫자가 아닌 값은 빠진 것으로 본다", () => {
  const got = readCounts({ ...NINE, bank: { dev: -1, prod: 2, dr: 3 } });
  assert.ok("missing" in got);
  assert.deepEqual(got.missing, ["bank.dev"]);
});

test("다운로드 주소는 끝 슬래시를 겹치지 않는다", () => {
  assert.equal(
    downloadUrl("https://sr.example.com/", "2026-09"),
    "https://sr.example.com/api/report/build?month=2026-09",
  );
});
...
test("작업 내역을 조회한다", async () => {
  const got = await getMonthlyWorkHandler(deps(), { month: "2026-09" });
  assert.ok(got.ok);
  assert.equal(got.month, "2026-09");
  assert.equal(got.rows.length, 1);
});

test("month 가 틀리면 조회하지 않는다", async () => {
  let called = false;
  ...
  assert.ok(!got.ok);
  assert.ok(!called, "잘못된 month 로 Jira 를 부르면 안 된다");
});

test("Atlassian 설정이 없으면 이유를 돌려준다", async () => {
  const got = await getMonthlyWorkHandler(deps({ atlassianConfig: () => null }), { month: "2026-09" });
  assert.ok(!got.ok);
...
test("저장된 수치가 없으면 채워야 할 아홉 칸을 알려 준다", async () => {
  const got = await getInstanceCountsHandler(deps(), { month: "2026-09" });
  assert.ok(got.ok);
...
test("수치를 주면 저장하고 주소를 돌려준다", async () => {
  ...
  assert.ok(got.ok);
  assert.equal(savedMonth, "2026-09");
  assert.equal(got.url, "https://sr.example.com/api/report/build?month=2026-09");
  assert.equal(got.previousFrom, "2026-08");
});
...
test("수치도 없고 저장된 것도 없으면 무엇을 물어야 할지 알려 준다", async () => {
  const got = await buildReportHandler(deps(), { month: "2026-09" });
  assert.ok(!got.ok);
  for (const field of REQUIRED_COUNTS) assert.ok(got.message.includes(field), field);
});

test("수치를 일부만 주면 빠진 칸을 짚어 준다", async () => {
  const got = await buildReportHandler(deps(), { month: "2026-09", bank: { dev: 1, prod: 2, dr: 3 } });
  assert.ok(!got.ok);
  assert.match(got.message, /central\.dev/);
});

// 빌드는 링크를 열 때 일어난다. 도구 안에서 하면 60초 타임아웃에 걸린다.
test("도구는 보고서를 만들지 않고 주소만 돌려준다", async () => {
  const got = await buildReportHandler(deps(), { month: "2026-09", ...NINE });
  assert.ok(got.ok);
