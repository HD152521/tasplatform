...
test("저장한 설정을 복호화해 되읽는다 (왕복)", () => {
  const db = freshDb();
  upsertLlmConfig(db, base);
  const got = getLlmConfig(db);
  assert.ok(got);
...
test("DB 에는 평문 비밀번호가 저장되지 않는다", () => {
  const db = freshDb();
  upsertLlmConfig(db, base);
  const row = db.prepare("SELECT secret_enc FROM llm_connections WHERE llm_id='default'").get() as { secret_enc: string };
  assert.ok(row.secret_enc.startsWith("v1:"));
  assert.ok(!row.secret_enc.includes("s3cret"));
  db.close();
});

test("화면용(meta)은 비밀번호를 노출하지 않고 존재 여부만 준다", () => {
  const db = freshDb();
  upsertLlmConfig(db, base);
  const meta = getLlmConfigMeta(db);
  assert.ok(meta);
...
test("비밀번호를 비워 수정하면 기존 값이 유지된다", () => {
  const db = freshDb();
  upsertLlmConfig(db, base);
  upsertLlmConfig(db, { ...base, name: "이름만 변경", secret: "" });
  const got = getLlmConfig(db);
...
test("keycloak 인데 토큰 URL 이 없으면 거부한다", () => {
  const db = freshDb();
  assert.throws(() => upsertLlmConfig(db, { ...base, tokenUrl: "" }), /토큰 발급 URL/);
  db.close();
});

test("http base_url 은 거부한다 (https 만)", () => {
  const db = freshDb();
  assert.throws(() => upsertLlmConfig(db, { ...base, baseUrl: "http://pais.ds.lab" }), /https/);
  db.close();
});

test("모델 ID 가 비면 거부한다", () => {
  const db = freshDb();
  assert.throws(() => upsertLlmConfig(db, { ...base, modelId: "" }), /모델 ID/);
  db.close();
});

test("DB 가 비면 환경변수(LLM_*)로 폴백한다", () => {
  const db = freshDb();
  assert.equal(getLlmConfig(db), null);
